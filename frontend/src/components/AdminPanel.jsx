...
const AdminPanel = () => {
  ...
  const handleAddVisualSpot = () => {
    setVisualSpots([...visualSpots, { id: Date.now(), ...tempCoordinates }]);
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="w-full lg:w-2/3 space-y-6">
            ...
            <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
              ...
              {/* TODO: Podepiąć pod ten input swój stan */}
              <input 
                type="text" 
          ...
          <div className="w-full lg:w-1/3 space-y-6">
              ...
              <div className="bg-blue-50 p-6 rounded-3xl border border-blue-100 shadow-sm animate-in slide-in-from-right-4">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    {/* TODO: Podpiąć pod ten input swój stan */}
                    <input 
                      type="text" 
                  ...
                  <div>
                    <label className="text-xs font-bold text-blue-800 block mb-2">Wybierz produkty ze sklepu</label>
                    <div className="space-y-2 max-h-48 overflow-y-auto bg-white p-2 rounded-xl border border-blue-200">
                      {/* TODO: Wygenerować tę listę dynamicznie na podstawie bazy */}
                      <label className="flex items-center gap-3 text-sm cursor-pointer hover:bg-slate-50 p-2 rounded-lg transition-colors border border-transparent hover:border-slate-100">
                        <input type="checkbox" className="w-4 h-4 accent-blue-600 cursor-pointer" />
                        <span className="truncate text-slate-700 font-medium">Przykładowa Wędka Pro</span>
                      </label>
                      <label className="flex items-center gap-3 text-sm cursor-pointer hover:bg-slate-50 p-2 rounded-lg transition-colors border border-transparent hover:border-slate-100">
                        <input type="checkbox" className="w-4 h-4 accent-blue-600 cursor-pointer" />
                        <span className="truncate text-slate-700 font-medium">Zestaw Spławików</span>
                      </label>
                    </div>
                  </div>
                  
                  {/* TODO: Podpiąć pod przycisk logikę zapisywania stanowiska */}
                  <button 
            ...
            <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
                    ...
                    {visualSpots.map((spot, idx) => (
                      <div key={spot.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100 group">
                        <span className="text-sm font-medium text-slate-700 flex items-center gap-3">
                          ...
                          <span className="truncate max-w-[150px]">Stanowisko dodane</span>
                        </span>
                        {/* TODO: Podpiąć logikę usuwania stanowiska ze swojego stanu */}
                        <button 
                          onClick={() => setVisualSpots(visualSpots.filter(s => s.id !== spot.id))}
              ...
              {/* TODO: Podpiąć logikę walidacji i wysyłki głównego formularza łowiska */}
