...
import { MapPin, Calendar, CheckCircle, ChevronDown, Clock } from 'lucide-react';

// TODO (Zbyszek): Te dwa łowiska zostają w kodzie, są jako podstawowe
...
// TODO (Zbyszek): Stanowiska także pobierane z bazy na podstawie tego co admin dodał
const DEFAULT_SPOTS = {
  1: [ 
...
// TODO (Zbyszek): Pobieranie dostępności stanowisk, finalnie będzie pobierne z bazy
    ...
    <div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-slate-100">
      <div className="bg-slate-900 p-6 flex flex-col sm:flex-row items-center justify-between text-white">
        ...
        <div className="relative w-full sm:w-72 text-slate-900">
          {/* TODO (Zbyszek): Zmodyfikuj mapowanie, żeby uwzględniało nowe łowiska z bazy */}
          <select 
      ...
      <div className="flex flex-col lg:flex-row">
        <div className="w-full lg:w-2/3 p-8 bg-slate-50 relative min-h-[500px] flex items-center justify-center">
          <div className="relative w-full max-w-3xl rounded-2xl overflow-hidden shadow-2xl border-4 border-white">
            <img 
              src={currentLake.image} 
              alt={currentLake.name} 
            ...
            {currentSpots.map((spot) => (
              ...
              >
                <div className={`
                  relative flex items-center justify-center w-10 h-10 rounded-full shadow-xl border-2 transition-colors
                  ${selectedSpot?.id === spot.id ? 'bg-blue-600 border-blue-300 scale-110 ring-4 ring-blue-500/50' : 'bg-slate-800 border-slate-600 hover:bg-slate-700 text-white'}
                `}>
        ...
        <div className="w-full lg:w-1/3 p-8 border-l border-slate-100 bg-white">
          {!selectedSpot ? (
            ...
            <div className="animate-in fade-in slide-in-from-right-4 duration-300 h-full flex flex-col">
              <div className="mb-6">
                <span className="inline-flex px-3 py-1 rounded-full text-xs font-bold mb-4 bg-blue-100 text-blue-700">
                  Wybrane stanowisko
                </span>
                ...
                <div className="flex-grow flex flex-col">
                    ...
                    <div className="flex gap-4">
                      <div className="w-1/2">
                        ...
                        <select 
                          ...
                          className="w-full p-2.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-blue-500 outline-none bg-white cursor-pointer"
                        >
                      ...
                      <div className="w-1/2">
                        ...
                        <select 
                          ...
                          disabled={!startTime}
                          className="w-full p-2.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-blue-500 outline-none bg-white cursor-pointer disabled:opacity-50"
                        >
                          <option value="" disabled>Wybierz...</option>
                          {ALL_HOURS.map(hour => (
                  ...
                  {startTime && endTime && (
                    <div className="mt-auto pt-6">
                      {/* TODO (Zbyszek): Pod ten przycisk podpiąć wysyłkę formularza */}
                      <button className="w-full flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white p-4 rounded-2xl font-bold text-lg transition-all hover:shadow-lg hover:-translate-y-1">
